...
configure_dns_netplan() {
  # 创建高优先级配置文件（排除lo接口）
  NETPLAN_FILE="/etc/netplan/99-dns-default.yaml"
  # 检测默认接口
...
# CentOS/RHEL (ifcfg)
configure_dns_centos() {
  default_interface=$(ip route | awk '/default/ {print $5}' | head -n 1)
  sed -i.bak 's/^\([[:space:]]*\)DNS\([0-9]=\)/\1#DNS\2/' "/etc/sysconfig/network-scripts/ifcfg-${default_interface}"
  echo "DNS1=$DNS_SERVER1" >> "$ifcfg"
  echo "DNS2=$DNS_SERVER2" >> "$ifcfg"
  systemctl restart NetworkManager || systemctl restart network
}

set_dns_server(){
  print_message "正在更新DNS"
  curr_os_name=$( get_linux_name )
  case "$curr_os_name" in
    ubuntu|debian)
      if systemctl is-active systemd-resolved; then
        # 使用变量替换 DNS 配置
        NEW_DNS="$DNS_SERVER1 $DNS_SERVER2"
        sed -i.bak 's/^\([[:space:]]*\)DNS=/#DNS=/' /etc/systemd/resolved.conf
        # 如果原文件无 DNS= 行，添加配置
        sudo sed -i '/^\[Resolve\]$/a DNS='"$NEW_DNS" /etc/systemd/resolved.conf
        # 重启服务
        sudo systemctl restart systemd-resolved
        #configure_dns_netplan
      else
        exit_now 1 "不支持的发行版: $curr_os_name"
      fi
  ...
  exit_on_code_failure $? "更新DNS失败"
  #print_message "尝试直接写入/etc/resolv.conf（临时生效）。"
...
print_message "4、系统DNS更新：error=手工配置DNS"
print_message "error=手工配置DNS，验证：curl -v --dns-servers 1.1.1.1 https://www.google.com/"
print_message "error=手工配置DNS，验证：curl -v --dns-servers 1.1.1.1 https://www.google.com/"
print_message "error=手工配置DNS，验证：curl -v --dns-servers 1.1.1.1 https://www.google.com/"
print_message "error=手工配置DNS，验证：nslookup www.google.com 8.8.8.8"
print_message "error=手工配置DNS，验证：nslookup www.google.com 8.8.8.8"
#curl -v --dns-servers 1.1.1.1 https://www.google.com/
